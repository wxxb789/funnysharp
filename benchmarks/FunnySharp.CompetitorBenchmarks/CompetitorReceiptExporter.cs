...
/// <c>eng/performance/competitor-baseline.json</c> instead of the main manifest. This project is isolated,
/// non-packable, and excluded from FunnySharp.slnx and every release artifact: pinned competitor packages
/// are performance evidence only, never API-compatibility evidence.
/// </summary>
internal sealed class CompetitorReceiptExporter : IExporter
{
    public string Name => nameof(CompetitorReceiptExporter);

    public void ExportToLog(Summary summary, ILogger logger)
    {
        var path = GetReceiptPath(summary);
        logger.WriteLineInfo($"Competitor performance receipt: {path}");
    }
    ...
    public IEnumerable<string> ExportToFiles(Summary summary, ILogger consoleLogger)
    {
        ...
        var manifestPath = Path.Combine(repositoryRoot, "eng", "performance", "competitor-baseline.json");
    ...
    private static string GetFingerprint(string repositoryRoot, JsonElement filesElement)
    {
        using var input = new MemoryStream();
        foreach (var fileElement in filesElement.EnumerateArray())
        {
            var relativePath = fileElement.GetString()
                ?? throw new InvalidOperationException("Fingerprint file entry was not a string.");
