...
| Carrier | `default` state | Documented | Tested | Goal 15 position (`docs/goals/archive/0015-goal.md:2`) |
| --- | --- | --- | --- | --- |
...
the canonical vocabulary" (`docs/goals/archive/0016-goal.md:2`), which allows either policy but not
...
| 14 | `Option<T>` | Case/factory; TryGetValue; Match ×2; Map; Bind; Filter; Zip; GetValueOr/GetValueOrElse/GetValueOrDefault; OrElse/OrElseWith; equality; ToString | keep | Goal 15 explicitly keeps default `None`; no throwing accessor; no `Value` property (C1-C4; `docs/goals/archive/0015-goal.md:2`, `docs/option.md:8-10`). |
...
| --- | --- | --- | --- | --- |
| G1 | `UnitResult<TError>` as the canonical success-or-failure-without-value carrier (`docs/goals/archive/0015-goal.md:2`) | Entire type; also UnitResult traversal and HTTP mapping | No `UnitResult` anywhere in `src/`, `tests/`, `examples/` (grep, no matches); `Result<TValue,TError>` forces a dummy `TValue` today | **adopt** (small struct mirroring `Result` case/factory/inspection but no `Map` value; add F3/F5/F10 integration only after the core shape is accepted) |
| G2 | Default/uninitialized `Result`/`Validation` must not masquerade as domain outcomes (`docs/goals/archive/0015-goal.md:2`) | Distinguishable default state | §2.1; `tests/FunnySharp.Tests/ResultTests.cs:8-19`, `ValidationTests.cs:8-19` | **redesign** (one decision point: explicit initialized flag/status, or default accessors throw, or keep + mandatory analyzer diagnostic; §5 rows 21 and 33) |
| G3 | No second absence carrier (`Maybe<T>`) and no `Validation.Bind` (`docs/goals/archive/0015-goal.md:2`) | Nothing — already satisfied | Dump type list; `docs/validation.md:30-33` | **reject** (record in the constitution as an explicit non-goal) |
| G4 | Fallible-function composition and folding/scan in one grammar (`docs/goals/archive/0016-goal.md:2`) | No Result/Option-aware `Compose`; no `Fold`/`Scan` | `FunctionExtensions` has no fallible composition; no fold/scan in any dump section; BCL `Aggregate` covers fold, not scan | **adopt** a narrow `Scan` (running aggregate) only; **adapt** fallible composition by documenting `Bind` as the canonical form instead of adding a pipeline hierarchy |
| G5 | Cardinality-sensitive and partition operations, non-empty guarantees (`docs/goals/archive/0017-goal.md:2`) | No `Partition`, `Single`/`ExactlyOne`, `NonEmpty`, exact-vs-truncating zip | `SequenceExtensions` and pipeline types only; no such members in dumps | **adopt** `Partition`; **defer** a non-empty carrier and exact-vs-truncating zip until a concrete consumer case (avoid a collection universe) |
| G6 | Traversal location/path context (`customers[17].addresses[2].postalCode`, `docs/goals/archive/0017-goal.md:2`) | No index/key/path on traversal failures | `docs/validation.md:43-66`; `SequenceExtensions.cs`, `AsyncSequenceExtensions.cs` produce only carrier payloads | **adopt** a minimal error-context shape for `Traverse`/`TraverseValueAsync` (index/key plus optional path segments), designed together with `UnitResult` so the API is not multiplied later |
